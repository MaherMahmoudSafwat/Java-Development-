...
public class MediumMathLevelQuiz extends MathQuizMenu
{
    ...
    {
        ArrayOfNumbers = new String[(int)(Math.random()*3)+2];
        ArrayOfNumbersOnly = new int[ArrayOfNumbers.length];
        for(int i = 0;i<ArrayOfNumbers.length;i++)
        {
            if(i==0)
            {
                ArrayOfNumbers[i] = String.valueOf((int) (Math.random() * 100));
    ...
    public static boolean GetFinalResult(String GetOperator,int UserInput)
    {
        int GetResult = 0;
        switch(GetOperator)
        {
